...
<#
    ...
    Vmware.Hv.Helper can be installed using the 'Install Hv.Helper module for Horizon View scripts' script. It can also be found on Github (see LINK). Download the module and place it in your systemdrive Program FilesWindowsPowerShellModules folder.
...
function Get-CUStoredCredential {
    ...
    $strCUCredFolder = "$([environment]::GetFolderPath('CommonApplicationData'))ControlUpScriptSupport"
    try {
...
function Connect-HorizonConnectionServer {
    param (    
        ...
        [parameter(Mandatory = $true,
            HelpMessage = "The Username object used for authentication.")]
        [string]$Username,
        [parameter(Mandatory = $true,
            HelpMessage = "The Password object used for authentication.")]
        [string]$Password
    )

    try {
        Connect-HVServer -Server $HVConnectionServerFQDN -User $Username -Password $Password
    }
    catch {
...
function Set-HorizonViewPoolProvisioningEnablement {
    ...
    if ($Enable) {
        try {
            Set-HVPool -Pool $HVPool -key 'desktopSettings.enabled' -value $True
...
[VMware.VimAutomation.HorizonView.Impl.V1.ViewObjectImpl]$objHVConnectionServer = Connect-HorizonConnectionServer -HVConnectionServerFQDN $strHVConnectionServerFQDN -Username $Username -Password $Password
